...
export const verifyMarchantToken = async (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_KEY, async (err, payload) => {
    ...
    if ( getUser.role !== 'marchant' ) return next(createError(403, `Only a merchant account have the right to perform this action`))
